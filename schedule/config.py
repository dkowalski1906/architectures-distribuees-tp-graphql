...
JSON_DATA_PATH = os.path.join(os.path.dirname(__file__), 'databases', 'times.json')
...
def get_schedule_repository():
    ...
    """
    if USE_MONGODB:
        from repositories.mongodb_repository import MongoScheduleRepository
        return MongoScheduleRepository(
            mongo_uri=MONGO_URI,
            db_name=MONGO_DB_NAME
        )
    else:
        from repositories.json_repository import JsonScheduleRepository
        return JsonScheduleRepository(json_file=JSON_DATA_PATH)
...
def print_config():
    ...
    print("=" * 60)
    print("CONFIGURATION DU SERVICE SCHEDULE")
    print("=" * 60)
    ...
    print(f"Schedule Service  : {SCHEDULE_HOST}:{SCHEDULE_PORT}")
