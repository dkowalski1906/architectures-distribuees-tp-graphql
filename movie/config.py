...
JSON_DATA_PATH = os.path.join(os.path.dirname(__file__), 'data', 'movies.json')
...
def get_movie_repository():
    ...
    """
    if USE_MONGODB:
        from repositories.mongodb_repository import MongoMovieRepository
        return MongoMovieRepository(
            mongo_uri=MONGO_URI,
            db_name=MONGO_DB_NAME
        )
    else:
        from repositories.json_repository import JsonMovieRepository
        return JsonMovieRepository(json_file=JSON_DATA_PATH)
...
def print_config():
    ...
    print("=" * 60)
    print("CONFIGURATION DU SERVICE MOVIE")
    print("=" * 60)
    ...
    print(f"Movie Service     : {MOVIE_HOST}:{MOVIE_PORT}")
